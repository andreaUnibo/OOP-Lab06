...
public class Robot {
	...
	public void moveDown() {
		moveToPosition(this.environment.getCurrPosX(), environment.getCurrPosY() - Robot.MOVEMENT_DELTA);
	}
	...
	public void moveLeft() {
		moveToPosition(this.environment.getCurrPosX() - Robot.MOVEMENT_DELTA, this.environment.getCurrPosY());
	}
	...
	private boolean moveToPosition(final int newX, final int newY) {
		...
		if (this.isBatteryEnoughToMove()) {
			...
			// this.log("Can not move to position(" + newX + "," + newY + "). Not enough
			// battery.");
